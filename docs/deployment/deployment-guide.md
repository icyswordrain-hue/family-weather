...
TTS rendering uses Google Cloud TTS when GCP credentials are available, with automatic fallback to Edge TTS (Microsoft's free WebSocket-based service).

| Environment | Provider | How |
|---|---|---|
| Modal | Google Cloud TTS | `GCP_SA_JSON` in `family-weather-secrets` → `_bootstrap_gcp_credentials()` sets `GOOGLE_APPLICATION_CREDENTIALS` |
| Cloud Run | Google Cloud TTS | Same mechanism if `GCP_SA_JSON` is available |
| Local | Edge TTS | Unless `local_data/service-account.json` exists |

Config in `config.py`: `TTS_PROVIDER` defaults to `"GOOGLE"` when `GOOGLE_APPLICATION_CREDENTIALS` is set, `"EDGE"` otherwise. Override with `TTS_PROVIDER` env var.

Voice names (overridable via env): `TTS_VOICE_ZH` (default `cmn-TW-Standard-A`), `TTS_VOICE_EN` (default `en-GB-Standard-C`).

Audio storage: TTS audio is stored in GCS at `gs://family-weather-dashboard/audio/{date}/{slot}_{lang}_{hash}.mp3`. A retention policy runs after each pipeline refresh — keeps the last 30 days plus 1 file per language on each 1st-of-month as a monthly snapshot.

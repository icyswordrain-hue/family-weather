...
_Last updated: 2026-03-14 (TTS diagnostic logging + stale entry cleanup)_
...
| Modal TTS | `https://icyswordrain--family-weather-engine-tts.modal.run` |
...
**GCS (Google Cloud Storage)** — Object storage. Stores broadcast history, TTS audio cache, and meal/location regen data. The bucket `family-weather-dashboard` has public read access (`allUsers:objectViewer`) — the browser plays TTS audio directly from GCS URLs without going through Flask.
...
**`family-weather-engine`** — The name of the Modal app. Contains three web endpoints: `health`, `refresh`, and `broadcast`. TTS synthesis runs inside `refresh()` — there is no separate TTS endpoint.
...
## Fix — 2026-03-11: Regen cycle triggering on every pipeline run

### Problem

`check_regen_cycle()` (`backend/pipeline.py`) scans the loaded history for the most
recent entry flagged as a regen event, returning `True` (trigger) when none is found.
The history passed to it came from `load_history(days=HISTORY_DAYS)` with
`HISTORY_DAYS=3`. Because `REGEN_CYCLE_DAYS=14`, any regen event older than 3 days
fell outside the window — `last_regen_date` was always `None` — and the check
returned `True` on every run. Effective regen frequency: every ~4 days instead of 14.

### Fix (commit `049de23`)

Load a separate, wider history slice specifically for the regen check:
...
`HISTORY_DAYS` (controls LLM prompt context size) is unchanged at 3.
`check_regen_cycle` logic is correct as written — it only needed a wider input window.

### Files changed
| File | Change |
|---|---|
| `app.py` | `_pipeline_steps()` — separate `regen_history = load_history(days=REGEN_CYCLE_DAYS+1)` fed to `check_regen_cycle` |

---

## Note — GCS Uniform Bucket-Level Access (UBL)

The bucket `family-weather-dashboard` uses **Uniform Bucket-Level Access** (enabled by default on buckets created since ~2022). This means:

- **Do NOT call `blob.make_public()`** — it sets per-object ACLs, which UBL rejects with `BadRequest: 400 Cannot use ACL API`.
- Public access is granted at the **bucket level** via `allUsers:objectViewer` (set during Step 2 of deployment setup). All uploaded objects are automatically public.
- `blob.public_url` resolves to `https://storage.googleapis.com/{bucket}/{path}` regardless of ACL state.

Verify UBL is enabled:
```
gcloud storage buckets describe gs://family-weather-dashboard --format="value(iamConfiguration.uniformBucketLevelAccess.enabled)"
```

---

## Note — TTS architecture (updated 2026-03-14)

TTS is **always eager** — audio is synthesized during every pipeline refresh (morning, midday, evening). There is no on-demand `/api/tts` endpoint and no `MODAL_TTS_URL`.

The full flow: Modal `refresh()` → `synthesise_with_cache()` → Google Cloud TTS → upload to GCS → `blob.public_url` stored in broadcast → browser plays GCS URL directly.

Cloud Run has only two Modal proxy URLs: `MODAL_REFRESH_URL` and `MODAL_BROADCAST_URL`. Audio is served directly from GCS, not through Cloud Run or Modal.

If TTS fails intermittently, check Modal logs (`modal app logs family-weather-engine`) for the diagnostic lines added in commit `3bc406a`:

- `TTS render: provider=..., has_gcp=..., text_len=..., lang=...`
- `Google Cloud TTS succeeded/failed`
- `Edge TTS also failed` (Edge TTS is blocked from datacenter IPs — Google Cloud TTS is the only viable provider in Modal)

...
- **Alternative A (Cards-Only):** Remove P1-P5, keep CARDS JSON, assemble TTS programmatically. Rejected because TTS quality degrades — concatenated card sentences sound robotic vs flowing prose.

- **Alternative B (Narration-Only):** Remove CARDS block, keep P1-P5 prose, derive cards from metadata. **Selected** — preserves natural TTS narration while eliminating duplicate output.

## Solution (Alternative B)

Expand the existing `---METADATA---` JSON with card-specific fields (taglines, text summaries, alert details) and remove the entire `---CARDS---` block. The parser derives the `cards` dict from metadata fields, keeping the downstream data shape identical.
...
|-----------|--------|-------|---------|
| System prompt (input) | ~800 tok | ~450 tok | ~350 tok |
| Narration prose (output) | ~600 tok | ~600 tok | 0 |
| CARDS JSON (output) | ~500 tok | 0 tok | ~500 tok |
| Metadata (output) | ~200 tok | ~300 tok | -100 tok |
| **Total** | **~2100 tok** | **~1350 tok** | **~750 tok (36%)** |

...
def synthesize_and_upload(
    ...
    if RUN_MODE == "CLOUD":
        ...
        bucket = None # Not used in LOCAL mode
    ...
    if RUN_MODE == "LOCAL":
        kids_url = _save_local_audio(kids_audio, kids_blob_name)
    else:
        # pyre-ignore[6]: bucket is not None in CLOUD mode
...
def _generate_edge_audio(narration_text: str, date_str: str) -> dict[str, str]:
    ...
    # 2. Kids Clip
    kids_text = _make_kids_text(narration_text)
    # Ana is a child voice
    kids_voice = "en-US-AnaNeural"
    kids_audio = _synthesize_edge(kids_text, kids_voice, rate="+10%")
    
    kids_blob_name = f"{prefix}/{GCS_KIDS_AUDIO_FILENAME}"
    kids_url = _save_local_audio(kids_audio, kids_blob_name)
    
    logger.info("Edge TTS complete. Full: %s | Kids: %s", full_url, kids_url)
    return {
        "full_audio_url": full_url,
        "kids_audio_url": kids_url,
    }
